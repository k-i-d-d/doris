...
MemTrackerLimiter* MemTrackerLimiter::create_tracker(int64_t byte_limit, const std::string& label, MemTrackerLimiter* parent, RuntimeProfile* profile) {
...
void MemTrackerLimiter::add_child_tracker(MemTrackerLimiter* tracker) {
    ...
    tracker->_child_tracker_it = _child_limiter_trackers.insert(_child_limiter_trackers.end(), tracker);
}

void MemTrackerLimiter::add_child_tracker(MemTrackerObserve* tracker) {
    std::lock_guard<SpinLock> l(_child_trackers_lock);
    tracker->_child_tracker_it = _child_observe_trackers.insert(_child_observe_trackers.end(), tracker);
}
...
void MemTrackerLimiter::transfer_to_relative(MemTrackerLimiter* dst, int64_t bytes) {
    DCHECK(dst->is_limited());
    if (id() == dst->id()) return;
    DCHECK_EQ(_ancestor_all_trackers.back(), dst->_ancestor_all_trackers.back()) << "Must have same ancestor";
    ...
    while (ancestor_idx > 0 && dst_ancestor_idx > 0 &&
           _ancestor_all_trackers[ancestor_idx - 1] == dst->_ancestor_all_trackers[dst_ancestor_idx - 1]) {
        DCHECK(!dst->_ancestor_all_trackers[dst_ancestor_idx - 1]->has_limit());
        --ancestor_idx;
        --dst_ancestor_idx;
    }
    MemTrackerLimiter* common_ancestor = _ancestor_all_trackers[ancestor_idx];
    release_local(bytes, common_ancestor);
    dst->consume_local(bytes, common_ancestor);
}

Status MemTrackerLimiter::check_limit(int64_t bytes) {
...
std::string MemTrackerLimiter::log_usage(int max_recursive_depth, const std::list<MemTrackerLimiter*>& trackers,
