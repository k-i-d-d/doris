...
MemTrackerObserve* MemTrackerObserve::create_tracker(const std::string& label, RuntimeProfile* profile) {
    ...
    if (parent_label.find_first_of("#") != parent_label.npos) {
        reset_label = fmt::format("[Observe]-{}#{}", label, parent_label.substr(parent_label.find_first_of("#"), -1));
    } else {
...
std::string MemTrackerObserve::log_usage(int64_t* logged_consumption) {
    ...
    std::string detail =
            "MemTracker log_usage Label: {}, Total: {}, Peak: {}";
    detail = fmt::format(detail, _label,
